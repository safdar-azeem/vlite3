...
<template>
  <div
    ...
    <!-- Sidebar Navigation -->
    <aside
      class="max-md:hidden w-full md:w-64! bg-card border-r border-border flex-shrink-0 flex flex-col h-auto md:h-screen sticky top-0 overflow-y-auto">
      <div
        class="p-6 border-b border-border flex items-center justify-between sticky top-0 bg-card z-10">
        ...
        <div v-for="cat in categories" :key="cat.name" class="space-y-1">
          ...
          <button
            ...
            <Icon
              :icon="item.icon"
              class="w-4 h-4"
              :class="activeTabId === item.id ? 'text-primary' : 'text-gray-400'" />
            {{ item.label }}
          </button>
        </div>
      </nav>
      ...
      <div class="p-4 border-t border-border">
        <div class="p-3 bg-blue-50/50 border border-blue-100 rounded-lg">
          <h4 class="text-xs font-semibold text-blue-900 uppercase tracking-wider mb-1">
            Playground
          </h4>
      ...
      <div class="max-w-5xl mx-auto px-6 py-10 md:py-12">
        ...
        <div
          ...
          <div class="flex items-center gap-2">
            <Button variant="outline" size="sm" icon="lucide:github">Documentation</Button>
