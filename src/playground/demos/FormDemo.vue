...
import sourceCode from './FormDemo.vue?raw'
  ...
  <div class="space-y-8">
    ...
    <div class="bg-white rounded-lg border border-border p-6">
        ...
        <div class="space-y-8">
          <div>
            ...
            <div class="flex flex-col lg:flex-row gap-6">
              ...
              <div
                v-if="submittedValues && submittedValues.tab === 'Login Form'"
          ...
          <div>
            ...
            <div class="flex flex-col lg:flex-row gap-6">
              ...
              <!-- Inline Submitted Values -->
              <div
                v-if="submittedValues && submittedValues.tab === 'Contact Form'"
                class="flex-1 max-w-md">
                <div class="bg-green-50 border border-green-200 rounded-lg p-4 h-full">
                  <div class="flex items-center gap-2 mb-3">
                    <Icon icon="lucide:check-circle" class="w-5 h-5 text-green-600" />
      ...
      <template v-else-if="activeTab === 'validation'">
        <div>
          <h3 class="text-lg font-semibold mb-1">Registration with Validation</h3>
          <p class="text-sm text-gray-500 mb-4">
            Comprehensive validation including regex patterns, password matching, age checks, and
            URL validation.
          </p>
          <div class="flex flex-col lg:flex-row gap-6">
            <div class="flex-1 max-w-2xl">
              <Form
                ...
                @onSubmit="(payload) => handleSubmit('Validation Form', payload)" />
            </div>
            <!-- Inline Submitted Values -->
            <div
              v-if="submittedValues && submittedValues.tab === 'Validation Form'"
              class="flex-1 max-w-md">
              <div class="bg-green-50 border border-green-200 rounded-lg p-4 h-full">
                <div class="flex items-center gap-2 mb-3">
                  <Icon icon="lucide:check-circle" class="w-5 h-5 text-green-600" />
                ...
                <div class="bg-white rounded border border-green-200 p-3 overflow-auto max-h-80">
                  <pre class="text-xs text-gray-700 whitespace-pre-wrap">{{
                    JSON.stringify(submittedValues.values, null, 2)
                  }}</pre>
                </div>
      ...
      <template v-else-if="activeTab === 'advanced'">
        <div>
          <h3 class="text-lg font-semibold mb-1">Conditional Fields & Computed Values</h3>
          <p class="text-sm text-gray-500 mb-4">
            Shows/hides fields based on selection (payment method) and auto-calculates totals using
          ...
          <div class="max-w-2xl">
            <Form
              ...
              @onSubmit="(payload) => handleSubmit('Advanced Form', payload)" />
          </div>
        </div>
      </template>
      ...
      <template v-else-if="activeTab === 'grouped'">
        <div>
          <h3 class="text-lg font-semibold mb-1">Grouped Form with Cards</h3>
          <p class="text-sm text-gray-500 mb-4">
            Form sections displayed as cards with headings and descriptions.
          </p>
          <div class="max-w-3xl">
            <Form
              :schema="[personalInfoSchema, addressSchema, preferencesSchema]"
              :groupsHeadings="['Personal Information', 'Address', 'Preferences']"
              ...
              @onSubmit="(payload) => handleSubmit('Grouped Form', payload)" />
          </div>
        </div>
      </template>
      ...
      <template v-else-if="activeTab === 'multistep'">
        <div>
          <h3 class="text-lg font-semibold mb-1">Multi-Step Wizard</h3>
          <p class="text-sm text-gray-500 mb-4">
            Step-by-step form with Timeline navigation. Each step validates before proceeding.
          </p>
          <div class="max-w-3xl">
            <Form
              :tabs="multiStepTabs"
              ...
              submitText="Create Account"
              @onSubmit="(payload) => handleSubmit('Multi-Step Form', payload)" />
          </div>
        </div>
      </template>
      ...
      <template v-else-if="activeTab === 'custom'">
        <div>
          <h3 class="text-lg font-semibold mb-1">Dynamic Custom Fields</h3>
          <p class="text-sm text-gray-500 mb-4">
            Add/remove dynamic rows like social links and team members using the CustomFields
            component.
          </p>
          <div class="max-w-4xl">
            <Form
              ...
              @onSubmit="(payload) => handleSubmit('Custom Fields Form', payload)" />
          </div>
        </div>
      </template>
      ...
      <template v-else-if="activeTab === 'uploads'">
        <div>
          ...
          <p class="text-sm text-gray-500 mb-4">
            Demonstration of avatar upload, single/multiple file uploads, and files within custom
            fields.
          </p>
          <div class="flex flex-col lg:flex-row gap-6">
            <div class="flex-1 max-w-2xl">
              <Form
                ...
                :values="{
                  ...
                  avatar:
                    'https://cdn.prod.website-files.com/666ad77562dfabab1eb27f63/67477a47fb8225b8ce5d1813_44cc44f1c9_15800_1_1732686878.webp',
                  idProof: 'https://storage.googleapis.com/vlite-bucket/yxycao/safdar-azeem-cv.pdf',
                  resume: 'https://storage.googleapis.com/vlite-bucket/rv65j/dummy.pdf',
                }"
                @onSubmit="(payload) => handleSubmit('Uploads', payload)" />
            </div>
            <!-- Inline Submitted Values -->
            <div
              v-if="submittedValues && submittedValues.tab === 'Uploads'"
              class="flex-1 max-w-md">
              <div class="bg-green-50 border border-green-200 rounded-lg p-4 h-full">
                <div class="flex items-center gap-2 mb-3">
                  <Icon icon="lucide:check-circle" class="w-5 h-5 text-green-600" />
    ...
    <Transition
      ...
      <div v-if="submittedValues" class="bg-green-50 border border-green-200 rounded-lg p-4">
        <div class="flex items-center justify-between mb-3">
          <div class="flex items-center gap-2">
            <Icon icon="lucide:check-circle" class="w-5 h-5 text-green-600" />
            <h4 class="font-semibold text-green-800">Form Submitted Successfully!</h4>
            <Badge variant="success" size="sm">{{ submittedValues.tab }}</Badge>
          </div>
          <Button variant="ghost" size="xs" icon="lucide:x" @click="clearSubmitted" />
        </div>
