...
A component to render properly formatted currency values. It uses the globally configured currency from <code>createVLite</code> or falls back to USD, and can be overridden per component.
</p>
</div>
...
<DemoSection title="Dynamic Global Configuration" :code="sourceCode">
  <div class="space-y-6">
    <div class="flex flex-col gap-2">
      <span class="text-sm font-medium">Current Global Currency: <strong class="text-primary">{{ config?.components?.price?.currency || 'USD' }}</strong></span>
      <p class="text-sm text-muted-foreground">Clicking these buttons uses <code>updateConfig()</code> to dynamically change the currency application-wide. The Price components below (without local currency props) will automatically react and update.</p>
    </div>
    ...
    <div class="p-6 border border-border rounded-xl bg-card shadow-sm flex flex-col sm:flex-row items-center gap-8 text-2xl font-black text-foreground">
      <div class="flex flex-col gap-1 items-center sm:items-start">
        <span class="text-xs font-semibold text-muted-foreground uppercase tracking-widest">Base Plan</span>
        <Price :value="29" />
      </div>
      <div class="flex flex-col gap-1 items-center sm:items-start">
        <span class="text-xs font-semibold text-muted-foreground uppercase tracking-widest">Pro Plan</span>
        <Price :value="99" />
      </div>
      <div class="flex flex-col gap-1 items-center sm:items-start">
        <span class="text-xs font-semibold text-muted-foreground uppercase tracking-widest">Enterprise</span>
        <Price :value="299" />
      </div>
    </div>
  </div>
...
<DemoSection title="Reactive Value" :code="sourceCode">
  <div class="space-y-4 max-w-sm">
    ...
    <div class="p-4 bg-muted/30 border border-border rounded-lg flex items-center justify-between">
