  ...
  <div class="space-y-8">
    ...
    <div>
      ...
      <p class="text-gray-500">
        Responsive masonry layout for displaying items with varying heights.
      </p>
    </div>
    ...
    <div class="p-4 bg-white border border-gray-200 rounded-lg shadow-sm space-y-4">
      <div class="flex flex-wrap gap-4 items-center justify-between">
        ...
        <!-- Actions -->
        <div class="flex items-center gap-2">
          <Button size="sm" variant="outline" icon="lucide:refresh-cw" @click="refresh"
            >Refresh</Button
          >
          <Button size="sm" variant="outline" icon="lucide:plus" @click="addItems"
            >Add Items</Button
          >
        </div>
      </div>
      ...
      <div class="flex flex-wrap gap-6">
        ...
        <div class="space-y-2">
          ...
          <div v-if="!useResponsive" class="flex items-center gap-2">
            ...
            <input type="range" v-model.number="fixedCols" min="1" max="10" class="w-32" />
          </div>
          <div v-else class="text-xs text-gray-400">Auto-responsive: sm:2, md:3, lg:4, xl:5</div>
        </div>
        ...
        <div class="space-y-2">
          ...
          <input type="range" v-model.number="gap" min="0" max="64" step="4" class="w-32" />
        </div>
    ...
    <!-- The Masonry Grid -->
    <div class="bg-gray-50/50 p-4 rounded-xl border border-dashed min-h-[500px]">
      <Masonry :data="items" :cols="cols" :gap="gap" v-slot="{ data }">
        <!-- Card -->
        <div
          ...
          <template v-else>
            <div class="flex items-center gap-3 mb-3">
              <div
                class="w-8 h-8 rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold text-xs ring-2 ring-white">
            ...
            <p class="text-gray-600 text-sm leading-relaxed">
              ...
              <span v-if="data.height > 300" class="block mt-2">
                Additional content to show variable height. Duis aute irure dolor in reprehenderit
                in voluptate velit esse cillum dolore.
              </span>
              <span v-if="data.height > 400" class="block mt-2 text-gray-400 italic">
                Even more content for very tall cards. Excepteur sint occaecat cupidatat non
                proident.
              </span>
            </p>
