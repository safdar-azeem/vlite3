...
const approvalSteps: TimelineStep[] = [
  ...
  { 
    ...
    status: 'Pending',
    user: { name: 'XYZ Builders', role: 'Project Contractor', img: 'https://i.pravatar.cc/150?u=1' } 
  },
  { 
    id: 3, 
    title: 'step', 
...
const reviewSteps: TimelineStep[] = [
  { id: 1, label: 'Step 1', title: 'Step completed, moving forward', date: '06 March 2026 1:26 am', status: 'Completed', logo: 'lucide:image' },
  { id: 2, label: 'Step 1', title: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit.', date: '06 March 2026 1:26 am', status: 'Approved', logo: 'lucide:image' },
  { id: 3, label: 'Step 1', title: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit.', date: '06 March 2026 1:26 am', status: 'In Progress', logo: 'lucide:image' },
]
</script>
...
<template>
  <div class="space-y-10">
    <div class="space-y-2">
      ...
      <p class="text-gray-500">Visualizes a sequence of events or steps. Built for extreme flexibility.</p>
    </div>
    ...
    <DemoSection title="Horizontal (Basic)" :code="sourceCode">
      <div class="flex items-center justify-between max-w-2xl mb-6">
        <div class="flex gap-2">
          ...
          <Button size="sm" variant="outline" :disabled="activeStep >= basicSteps.length" @click="activeStep++">Next</Button>
        </div>
      </div>
    ...
    <DemoSection title="Vertical: Approval Workflow" :code="sourceCode">
      <p class="text-sm text-muted-foreground mb-4">Using `indicator-type="number"`, `line-style="dashed"`, and `#body` slot injections.</p>
      <div class="p-6 border rounded-lg max-w-xl bg-background">
        ...
        >
          <template #body="{ step }">
            <div class="mt-4 flex items-center gap-4 p-4 bg-muted/20 rounded-xl border border-border shadow-sm">
              <img v-if="step.user.img" :src="step.user.img" class="w-12 h-12 rounded-lg object-cover bg-muted" />
              <div v-else class="w-12 h-12 rounded-lg bg-muted flex items-center justify-center text-muted-foreground font-bold">
                {{ step.user.initials }}
              </div>
    ...
    <DemoSection title="Vertical: Complex Content Cards" :code="sourceCode">
      <p class="text-sm text-muted-foreground mb-4">Taking over the entire `#content` slot for complete layout freedom.</p>
      <div class="p-6 max-w-3xl">
        ...
        >
          <template #content="{ step }">
            <div class="border border-border rounded-xl p-5 bg-card shadow-sm mb-2 hover:shadow-md transition-shadow">
              <div class="flex justify-between items-start">
                ...
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border"
                  :class="[
                    step.status === 'Completed' ? 'bg-primary-light text-primary border-primary/20' : '',
                    step.status === 'Approved' ? 'bg-success-light text-success border-success/20' : '',
                    step.status === 'In Progress' ? 'bg-primary-light text-primary border-primary/20' : ''
                  ]">
                  <Icon v-if="step.status === 'Approved'" icon="lucide:check" class="h-3 w-3" />
                  <Icon v-else-if="step.status === 'In Progress'" icon="lucide:loader-2" class="h-3 w-3 animate-spin" />
                  <Icon v-else-if="step.status === 'Completed'" icon="lucide:history" class="h-3 w-3" />
                  {{ step.status }}
                </span>
              </div>
