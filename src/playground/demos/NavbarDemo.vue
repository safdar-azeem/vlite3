...
<DemoSection title="App Layout Mode (Full Structure)" :code="sourceCode">
  <p class="text-sm text-gray-500 mb-4">
    Pass <code>header</code> and <code>main</code> slots to transform the Navbar into a full-page layout wrapper.
  </p>
  <div class="border rounded-lg overflow-hidden h-[500px] relative z-0 flex flex-col w-full">
    <Navbar variant="sidebar" class="bg-white border-r">
      <template #header="{ toggle }">
        <div class="h-16 border-b bg-white flex items-center justify-between px-6 w-full shadow-sm">
          <div class="flex items-center gap-4">
            <Button variant="ghost" size="sm" icon="lucide:menu" class="md:hidden -ml-2" @click="toggle" />
            <div class="font-bold text-lg flex items-center gap-2">
              <div class="w-8 h-8 rounded bg-primary text-white flex items-center justify-center">B</div>
              Builto
            </div>
          </div>
          <div class="flex items-center gap-3">
            <Input placeholder="Search..." icon="lucide:search" size="sm" rounded="full" class="bg-gray-100 hidden sm:flex w-64" />
      ...
      <template #logo>
        <div class="hidden md:block px-2 mt-4 text-xs font-semibold text-gray-400 uppercase tracking-wider">
          Navigation
        </div>
      </template>
      ...
      <template #main>
        <div class="p-6 h-full bg-gray-50">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div class="bg-white p-6 rounded-xl border shadow-sm h-32 flex flex-col justify-center items-center text-gray-400">Card 1</div>
            <div class="bg-white p-6 rounded-xl border shadow-sm h-32 flex flex-col justify-center items-center text-gray-400">Card 2</div>
            <div class="bg-white p-6 rounded-xl border shadow-sm h-32 flex flex-col justify-center items-center text-gray-400 md:col-span-2 lg:col-span-1">Card 3</div>
          </div>
        </div>
      </template>
    </Navbar>
