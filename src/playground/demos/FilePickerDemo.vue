  ...
  <div class="space-y-8 p-4 max-w-3xl mx-auto">
    <div>
      ...
      <p class="text-gray-500 mb-6">
        A versatile file selection component with drag & drop, multi-select, and custom trigger
        support.
      </p>
    </div>
    ...
    <DemoSection title="Base64 Output" :code="sourceCode">
      <div class="p-4 border rounded-xl bg-gray-50/50">
        <FilePicker
          ...
          @error="handleError" />
        <div v-if="base64Files && base64Files.base64" class="mt-4">
          <p class="text-xs text-gray-500 mb-2">Preview from Base64:</p>
          <img :src="base64Files.base64" class="h-20 w-auto rounded border shadow-sm" />
        </div>
      </div>
    </DemoSection>
          ...
          <template #trigger="{ trigger, files, isLoading }">
            <div class="flex items-center gap-4">
              <Button :loading="isLoading" icon="lucide:upload" @click="trigger">
                {{ files && files.length ? 'Change File' : 'Upload File' }}
              </Button>

              <div
                v-if="files && files.length > 0"
                class="flex items-center gap-2 text-sm text-gray-700">
                <Icon icon="lucide:check-circle" class="text-success h-4 w-4" />
                {{ files[0].fileName }}
              </div>
              <div v-else class="text-sm text-gray-400 italic">No file selected</div>
            </div>
          </template>
        </FilePicker>
      </div>
