...
/** When true, applies the compact print-friendly layout to the live preview */
const isCompact = ref(false)

const variants: { value: InvoiceVariant; label: string; description: string }[] = [
  {
    value: 'Variant1',
    label: 'Variant 1 — Enterprise',
    description: 'Modern two-column with full brand header',
  },
  {
    value: 'Variant2',
    label: 'Variant 2 — POS Receipt',
    description: 'Compact thermal-print style receipt',
  },
  {
    value: 'Variant3',
    label: 'Variant 3 — Compact Table',
    description: 'E-commerce packing slip with tight layout',
  },
  {
    value: 'Variant4',
    label: 'Variant 4 — Bold Brand',
    description: 'Vibrant gradient hero header invoice',
  },
]
...
const ecomInvoice: InvoiceData = {
  ...
  items: [
    ...
    },
    {
      ...
      description: 'Space Grey - Multi-Touch Surface',
      quantity: 1,
      price: 79,
      total: 79,
    },
    {
      id: 3,
      name: 'USB-C Hub Pro 8-in-1',
      description: '4K HDMI, 100W PD, SD/microSD, 3xUSB-A',
      quantity: 2,
      price: 49.99,
      total: 99.98,
    },
    {
      ...
      description: '3-Year AppleCare+ equivalent plan',
      quantity: 1,
      price: 299,
      total: 299,
    },
...
const invoicePropsFields = [
  {
    ...
    type: 'InvoiceData',
    default: '—',
    desc: 'The full invoice data object (required).',
  },
  {
    name: 'variant',
...
const invoiceDataFields = [
  { name: 'brandLogo', type: 'string?', desc: 'URL to the brand logo image.' },
  {
    name: 'brandTitle',
    type: 'string?',
    desc: 'Display title e.g. Invoice, Receipt, Purchase Order.',
  },
  { name: 'brandName', type: 'string?', desc: 'Company / brand name.' },
  {
    name: 'companyInfo',
    type: 'InvoiceCompanyInfo?',
    desc: 'Issuer address, email, phone, tax ID, etc.',
  },
  {
    name: 'invoiceNumber',
    type: 'string',
    desc: 'Unique invoice / receipt identifier (required).',
  },
  { name: 'status', type: 'string?', desc: 'Status label e.g. Paid, Pending, Processing.' },
  {
    name: 'issuedDate',
    type: 'string | Date?',
    desc: 'Date the invoice was issued. Formatted via DateTime.',
  },
  ...
  },
  {
    ...
    desc: 'Array of line items with name, qty, price, total (required).',
  },
  {
    name: 'totals',
    type: 'InvoiceTotal[]',
    desc: 'Summary rows — subtotal, tax, discount, grand total, etc. (required).',
  },
  ...
  { name: 'notes', type: 'string?', desc: 'Additional notes or memo text for the receiver.' },
  {
    name: 'footerText',
    type: 'string?',
    desc: 'Small footer text, e.g. terms, website, contact info.',
  },
]
...
const invoiceTotalFields = [
  ...
  { name: 'value', type: 'number', desc: 'Numeric monetary value, formatted via Price component.' },
  {
    name: 'isGrandTotal',
    type: 'boolean?',
    desc: 'Marks this row as the grand total — rendered larger & bold with a separator line above.',
  },
  {
    name: 'isSubtotal',
    type: 'boolean?',
    desc: 'Semantic marker for the subtotal row (used for styling in some variants).',
  },
  ...
  <div class="space-y-12 pb-20">
    <div class="space-y-4 pb-8 border-b border-border/50">
      ...
      <p class="text-lg text-muted-foreground w-full max-w-[80%]">
        A flexible, fully-typed invoice component with 4 visual variants. Supports brand identity,
        customer info, line items, totals, QR codes, barcodes, notes, and a footer. Works seamlessly
        with the library's <code class="text-primary font-mono text-sm">Price</code> and
