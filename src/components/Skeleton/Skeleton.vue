...
import { adjustColor, getRegisteredBones, resolveResponsive, getGlobalConfig } from './shared'
...
const resolvedColor = computed(() =>
  isDark.value
    ? (props.darkColor ?? globalConfig.darkColor ?? 'rgba(255,255,255,0.06)')
    : (props.color ?? globalConfig.color ?? 'rgba(0,0,0,0.08)')
)

const effectiveBones = computed(() =>
  dynamicBones.value ?? props.initialBones ?? (props.name ? getRegisteredBones(props.name) : undefined)
)
...
const showFallback = computed(() => props.loading && !activeBones.value && !transitioning.value && !props.auto)
...
function captureDynamicBones() {
  nextTick(() => {
    requestAnimationFrame(() => {
      if (fixtureRef.value) {
        dynamicBones.value = snapshotBones(fixtureRef.value, props.name || 'auto-skeleton', props.snapshotConfig)
      }
    })
  })
}
