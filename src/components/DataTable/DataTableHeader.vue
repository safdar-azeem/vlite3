...
const sizeClass = computed(() => {
  ...
  if (props.size === 'sm') return `py-2! ${props.cellBordered ? 'px-3!' : ''}`
  if (props.size === 'lg') return `py-4! ${props.cellBordered ? 'px-4!' : ''}`
  return `py-3! ${props.cellBordered ? 'px-3.5!' : ''}`
})
</script>
    ...
    <div class="flex items-center gap-2 space-x-2" :class="alignClass">
      ...
      <span v-if="isSortEnabled" class="w-4 h-4 shrink-0">
        <Icon
          ...
          :class="[
            isSorted ? 'opacity-100 text-foreground' : 'opacity-0 group-hover/th:opacity-100',
          ]" />
      </span>
    </div>
  </th>
