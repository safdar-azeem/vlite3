  ...
  <div class="v-invoice-v1 bg-background text-foreground rounded-xl border border-border overflow-hidden shadow-sm">
    ...
    >
      <div :class="compact ? 'space-y-3' : 'space-y-4'">
        ...
        <div>
          <h2 v-if="d.brandName" :class="compact ? 'text-lg font-bold' : 'text-xl font-bold'">{{ d.brandName }}</h2>
      ...
      <div class="text-left md:text-right shrink-0" :class="compact ? 'space-y-3' : 'space-y-4'">
        <div>
          ...
          <p class="font-medium mt-1 text-muted-foreground" :class="compact ? 'text-base' : 'text-lg'">#{{ d.invoiceNumber }}</p>
        </div>
    ...
    <div :class="compact ? 'p-6 space-y-6' : 'p-6 sm:p-8 space-y-8'">
      ...
      <div v-if="d.items && d.items.length" class="overflow-x-auto rounded-lg border border-border">
        <table class="w-full text-left whitespace-nowrap" :class="compact ? 'text-xs' : 'text-sm'">
          <thead class="bg-muted text-muted-foreground uppercase font-semibold" :class="compact ? 'text-xs' : 'text-xs'">
            <tr>
              <th scope="col" :class="compact ? 'px-4 py-2' : 'px-6 py-3'">Description</th>
              <th scope="col" class="text-right" :class="compact ? 'px-4 py-2' : 'px-6 py-3'">Qty</th>
              <th scope="col" class="text-right" :class="compact ? 'px-4 py-2' : 'px-6 py-3'">Price</th>
              <th scope="col" class="text-right" :class="compact ? 'px-4 py-2' : 'px-6 py-3'">Total</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-border">
            <tr v-for="(item, index) in d.items" :key="item.id || index" class="hover:bg-muted/50 transition-colors">
              <td :class="compact ? 'px-4 py-2 white-space-normal' : 'px-6 py-4 white-space-normal'">
              ...
              <td class="text-right tabular-nums" :class="compact ? 'px-4 py-2' : 'px-6 py-4'">{{ item.quantity }}</td>
              <td class="text-right tabular-nums" :class="compact ? 'px-4 py-2' : 'px-6 py-4'"><Price :value="item.price" /></td>
              <td class="text-right tabular-nums font-medium" :class="compact ? 'px-4 py-2' : 'px-6 py-4'"><Price :value="item.total" /></td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="flex flex-col-reverse md:flex-row justify-between" :class="compact ? 'gap-4 pt-2' : 'gap-8 pt-4'">
        ...
        <div class="w-full md:w-1/2" :class="compact ? 'space-y-3' : 'space-y-6'">
          <div v-if="d.notes" class="space-y-1">
            <h4 class="font-semibold text-muted-foreground uppercase tracking-wider text-sm">Notes</h4>
            <div
