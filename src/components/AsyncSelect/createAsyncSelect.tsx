...
export const createAsyncSelect = <T extends { id?: string | number }>(
  ...
  return defineComponent({
    ...
    props: {
      ...
      addNewConfig: { type: Object as PropType<AddNewConfig> },
    },
    ...
    setup(props, { attrs, emit }) {
      ...
      const activeMapper = computed(() => props.optionsMapper || config.optionsMapper)
      ...
      const hasMore = computed(() => {
        ...
        return pageInfo ? pageInfo.currentPage < pageInfo.totalPages : false
      })
      ...
      watch(
        ...
        (rawData) => {
          if (!rawData) return
          const newItems = Array.isArray(rawData) ? rawData : rawData.items
          if (!newItems) return
      ...
      const options = computed(() => items.value.map(mapItem).filter(Boolean) as any[])
      ...
      const fetchSelected = async (ids: any[]) => {
        ...
        const currentValues = new Set(options.value.map((o: any) => o.value))
        const missingIds = validIds.filter((id) => !currentValues.has(id))
      ...
      const renderAddNew = () => {
        if (!props.addNewConfig) return null

        return (
          <div class="px-1 pb-1 pt-1 mt-1 border-t border-border bg-body sticky bottom-0 z-10">
            <Modal
              title={props.addNewConfig.label || 'Add New'}
              titleI18n={props.addNewConfig.labelI18n}
              body={props.addNewConfig.component}
              bodyProps={props.addNewConfig.props}
              onClose={() => {
                refetch()
              }}>
              {{
                trigger: () => (
                  <Button
                    ...
                    size="sm"
                    icon="lucide:plus"
                    text={props.addNewConfig!.label || 'Add New'}
                  />
                ),
              }}
            </Modal>
      ...
      return () => {
        ...
        return (
          <Dropdown
            ...
            onOnSelect={(payload: any) => handleChange(payload.value, payload.data)}>
            {slots}
          </Dropdown>
        )
      }
