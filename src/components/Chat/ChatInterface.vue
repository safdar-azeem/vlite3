...
watch(() => props.data.length, async (newLen, oldLen) => {
...
const startEditing = (msg: ChatMessage) => {
  ...
  if (textareaRef.value) {
    ...
    handleInput() // Adjust height for the new text
  }
}
...
<template>
  <div class="flex flex-col w-full h-full flex-1 min-h-0 bg-transparent">
    ...
    >
      ...
      <div v-if="isLoadingMore" class="py-2 mb-2 text-center text-xs text-muted-foreground flex justify-center items-center gap-2">
      ...
      <div v-if="!data.length && !isLoadingMore" class="flex items-center justify-center h-full text-muted-foreground text-sm">
        No messages yet
      </div>
    ...
    <div class="p-3 shrink-0 bg-transparent flex flex-col gap-1">
      ...
      <div v-if="editingMessage" class="flex items-center justify-between px-3 py-1.5 bg-muted/50 rounded-lg text-xs text-muted-foreground border border-border animate-in fade-in slide-in-from-bottom-2">
        <div class="flex items-center gap-2">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 text-primary" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg>
          <span class="truncate max-w-[250px] md:max-w-[400px]">Editing: {{ editingMessage.text }}</span>
        </div>
        <button @click="cancelEdit" class="hover:text-foreground p-0.5 rounded-full hover:bg-muted transition-colors" aria-label="Cancel editing">
      ...
      <div class="relative flex items-end gap-2 bg-card border border-border rounded-xl p-2 shadow-sm focus-within:border-primary transition-colors">
        <textarea
          ref="textareaRef"
          v-model="inputText"
          :placeholder="placeholder"
          class="flex-1 max-h-[120px] min-h-[40px] w-full resize-none bg-transparent px-3 py-2 text-sm text-foreground outline-none placeholder:text-muted-foreground scrollbar-thin"
          rows="1"
